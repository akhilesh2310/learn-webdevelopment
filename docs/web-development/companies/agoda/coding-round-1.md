...
* **Evaluation:**
  * **r1**: Pairs are `(100, 105) -> diff 5 <= 10` (**Retry**) and `(105, 200) -> diff 95 > 10` (No).
...
* **Evaluation:**
  * **a**: Consecutive logs are at `10` and `14 -> diff 4 <= 5` (**Retry**).
  * **b**: Consecutive logs are at `12` and `20 -> diff 8 > 5` (No).
* **Output:** 1
...
* **Input:** s = "110100"
* **Optimal Moves:**
  1. Move the '1' at index 1 past the zero at index 2 -> "101100" (`cost = 1 + 1 = 2`).
  2. Move the '1' at index 0 past the zero at index 1 -> "011100" (`cost = 1 + 1 = 2`).
...
* **Input:** s = "101"
* **Optimal Moves:**
  * Move the first '1' past the single zero to bunch up with the second '1' -> "011".
