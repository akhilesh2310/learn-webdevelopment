---
title: JWT, CSRF, and Token Storage
sidebar_position: 2
---
...
Since React runs completely in the browser and cannot use traditional server-side template tags like Django's:

```django
{% csrf_token %}
```

The recommended solution is the **Double Submit Cookie pattern**.
...
:::note Project-specific follow-up
...
:::warning
While safe from CSRF, localStorage is highly vulnerable to XSS attacks. If an attacker injects malicious JavaScript, they can steal the JWT instant
:::
